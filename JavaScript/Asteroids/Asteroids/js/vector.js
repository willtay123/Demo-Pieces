class Vector {
    ...
    divide(pDivider) {
        ...
        if (pDivider === 0) {
            //would lead to infinity, how to handle? return a big?
            newX = 0;
            newY = 0;
        }
        else {
            newX = this.getX() / pDivider;
            newY = this.getY() / pDivider;
        }
    ...
    angle() {
        var angle = Math.atan2(this.getY(), this.getX());
        return angle;
    }

    inverse() {
        var inverse = this.multiply(-1);
